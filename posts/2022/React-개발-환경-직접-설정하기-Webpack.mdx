...
배포할 때는 <strong>시작점이 되는 파일(entry)</strong>로부터 모든 의존하는 모듈들을 합쳐 하나의 파일로 만든다.
...
<strong>말 그대로 시작점(entry)이 되는 모듈</strong>을 지정한다.
...
번들링 된 결과물이 저장될 <strong>위치</strong>를 지정한다.

<strong>번들링 된 결과물의 이름</strong>을 지정한다.
